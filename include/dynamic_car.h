...
public:
    DynamicCar() {
        ...
        _w_boundary_constrains = 0.0000001;
        _w_kappa_constrains = 0.000001;
    ...
    virtual double cost(const VectorXd& x, const VectorXd& u, 
        ...
        double cost_constrains = constrains_cost(x, u, road_reference_line, road_left_side, road_right_side);

        cost = cost_ref + cost_comfort + cost_constrains;
        return cost;
    }

    virtual double unify_cost(const VectorXd& x, 
            const VectorXd& u, 
            const VecofVec2d& road_reference_line,
            const VecofVec2d& road_left_side,
            const VecofVec2d& road_right_side) override {
    ...
    // constrains cost
    virtual double constrains_cost(const VectorXd& x, 
            const VectorXd& u, 
            const VecofVec2d& road_reference_line,
            const VecofVec2d& road_left_side,
            const VecofVec2d& road_right_side) override {
        ...
        piecewise_function(boundary_constrains, k_constrains);

        double cost_boundary_constrains = 0.0f;
        cost_boundary_constrains = _w_boundary_constrains * (pow(boundary_constrains[0], 4) + 
                pow(boundary_constrains[1], 4) + pow(boundary_constrains[2], 4) + pow(boundary_constrains[3], 4));


        double cost_kappa_constrains = 0.0f;
        cost_kappa_constrains = _w_kappa_constrains * (pow(k_constrains[0], 4) + pow(k_constrains[1], 4));

        double cost_constrains = 0.0f;
        cost_constrains = cost_boundary_constrains + cost_kappa_constrains;

        return cost_constrains;
    }

    // piecewise function 分段函数
    void piecewise_function(VectorXd& boundary_constrains, VectorXd& kappa_constrains) {
        for(int i=0; i<boundary_constrains.size(); i++) {
            if(boundary_constrains[i] <= 0) boundary_constrains[i] = 0;
        }
        for(int j=0; j<kappa_constrains.size(); j++) {
            if(kappa_constrains[j] <= 0) kappa_constrains[j] = 0;
        }
    }
...
private:
    ...
    double _w_boundary_constrains;
    double _w_kappa_constrains;
    Vector2d x_f, x_r;
}; 
