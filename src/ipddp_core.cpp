...
double IPDDP::init_traj(const VectorXd& x_0, 
    ...
    std::string compress_road_path = "/home/dmz/Baidu/iLQR_barrier_function/compress_road_path_data"; 
...
void IPDDP::generate_trajectory(int index_data) {
    ...
    cout << "iter = " << iter << endl;
    std::string outputPath = "/home/dmz/Baidu/iLQR_barrier_function/road_path_planning_data";
    std::string fileName = outputPath + "/" +"iLQR_result" + std::to_string(index_data) + ".csv";
    //cout << fileName << endl;
    ...
    double path_cost = 0;
    for(int i=0; i<T; i++) {
        path_cost += model->unify_cost(xs[i], us[i], road_reference_line,road_left_side,road_right_side);
    }
...
void IPDDP::forward_pass() {
    ...
    for (int i=0; i<Alpha.size(); i++) {
        ...
        if (failed!=0) {
            ...
            double new_cost = 0;
            for(int i=0; i<T; i++) {
                new_cost += model->cost(x_new[i], u_new[i], road_reference_line, road_left_side, road_right_side);
            }
    ...
    for (int i=(T-1); i>=0; i--) {
        ...
        if (alg.infeasible) {
            ...
            if(llt_infeasible.info()==Eigen::Success) {
                ...
                // if(bp_regularization==24){
                //     cout <<"id = " << i << ", Quureg = " << Quu_reg << ", Quu = " << Quu
                //             << ", Qsusu = " << Qsu.transpose()*SY_inv*Qsu <<endl;
                // }
                // bp_failed = 1;
                break;
            }
...
void IPDDP::output_road_to_csv(const std::string filename) {
    ...
    for(int t=0; t<road_reference_line.size(); t++) {
        ...
        if(t != road_reference_line.size()-1) {
            fprintf(road_xu, "\n");
        }
    }
...
double IPDDP::initialroll(const VectorXd& x0, const VecOfVecXd& u) {
    ...
    for(int t=0; t<T; t++) {
        ...
        constrains[t] = model->constrains_function(x_curr, 
                ...
                road_left_side,
                road_right_side);
        total_cost += model->cost(x_curr, u_curr, road_reference_line, road_left_side, road_right_side);
